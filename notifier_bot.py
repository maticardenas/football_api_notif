...
async def help(update: Update, context):
    text = f"{Emojis.WAVING_HAND.value}Hola {update.effective_user.first_name}!\n\n" \
...
async def next_match(update: Update, context):
    ...
    if validated_input:
        ...
        text, photo = team_fixtures_manager.get_next_team_fixture_text(update.effective_user.first_name)
...
async def last_match(update: Update, context):
    ...
    if validated_input:
        ...
        current_season = date.today().year
        team_fixtures_manager = TeamFixturesManager(current_season, team.id)
        text, photo = team_fixtures_manager.get_last_team_fixture_text(update.effective_user.first_name)
        await context.bot.send_photo(chat_id=update.effective_chat.id, photo=photo, caption=text, parse_mode="HTML")
